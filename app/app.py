...
def predict():
    try:
        ...
        # Encode inputs (example encoding)
        try:
            encoded = [
            ...
            flash("Invalid selection detected:{str(e)}","error")
            return render_template('index.html')


        # Manual scaling example
        scaled_encoded = encoded.copy()
        scaled_encoded[1] = (encoded[1]-1)/3  # Age
        scaled_encoded[5] = encoded[5]/2      # Severity
        scaled_encoded[9] = (encoded[9]-1)/2  # When diagnosed
